        ...
        if not shutil.which("dirsearch"):
            ...
            if shutil.which("gobuster") and os.path.exists(wordlist):
                ...
                try:
                    ...
                    res = ToolResult(proc.returncode, proc.stdout, proc.stderr, cmd, duration)
                except Exception as e:
                    res = ToolResult(1, "", str(e), cmd, 0.0)
            else:
                return DirsearchResult(target_url=url, entries=[], raw=ToolResult(1, "", "Neither dirsearch nor gobuster found", "", 0.0))
        else:
